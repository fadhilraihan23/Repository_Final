...
function dataUser() {
	...
	if (isset($_SESSION['id_user'])) {
		...
		return mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT * FROM user WHERE id_user = '$id_user'"));
	} else {
		echo "
		  <script>
...
function ubahProfile($data) {
	...
	mysqli_query($koneksi, "UPDATE user SET username = '$username', nama_lengkap = '$nama_lengkap', photo_profile = '$photo_profile' WHERE id_user = '$id_user'");
...
function tambahUser($data) {
	global $koneksi;
	$username = htmlspecialchars(addslashes($data['username']));
	$email = htmlspecialchars(addslashes($data['emailUser']));
	$password = htmlspecialchars(addslashes($data['password']));
	$epassword = password_hash($password, PASSWORD_DEFAULT);
	mysqli_query($koneksi, "INSERT INTO user VALUES ('', '$username', '$email', '$epassword')");
	return mysqli_affected_rows($koneksi);
}